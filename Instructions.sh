...
# Install Formulae:
brew install asciinema bat btop burp-suite cowsay coreutils discord fd fontconfig fortune fzf gh gnupg git go hugo imagemagick lazygit lsd m-cli mas neofetch neovim nmap notion node podman pv radare2 ripgrep shellcheck ciphey spotify starship sampler thefuck tmux tldr telegram tree vim wget weechat alfred alt-tab amethyst android-studio appcleaner cheatsheet element fig hyper insomnia maccy minecraft neovide numi obsidian onefetch onyx postman session steam topnotch 1. chezmoi socat telnet lua httpie tor cmatrix gawk moreutils findutils renameutils rename rlwrap rclone nikto exif exiftool ack wireshark pwntools zeek monit masscan zmap apktool smali ghidra bettercap bfg binutils binwalk cifer dex2jar dns2tcp fcrackzip foremost hashpump hashcat gobuster hydra john httpx nuclei ffuf exploitdb knock netpbm pngcheck sqlmap tcpflow tcpreplay tcptrace xpdf xz aria2 iperf3 ucspi-tcp owasp-zap yt-dlp git-lfs gnu-sed speedtest-cli proxychains-ng aircrack-ng cloudflare-warp visual-studio-code hiddenbar google-chrome reattach-to-user-namespace zsh-async zsh-autosuggestions zsh-completions zsh-fast-syntax-highlighting zsh-git-prompt zsh-history-substring-search zsh-lovers zsh-navigation-tools zsh-vi-mode zsh-you-should-use
